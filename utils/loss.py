...
def wasser_loss_diff(factors_probability1, factors_probability2):
    B, C, N_bins = factors_probability1.shape
    factors_probability1 = torch.nn.Softmax(dim=-1)(factors_probability1)
    factors_probability2 = torch.nn.Softmax(dim=-1)(factors_probability2)
    
    factors_cdf1 = torch.cumsum(factors_probability1, dim=-1)
    factors_cdf2 = torch.cumsum(factors_probability2, dim=-1)

    diff_abs_cdf = torch.abs((factors_cdf1 - factors_cdf2))
    integral_diff = torch.sum(diff_abs_cdf/N_bins, dim=-1)
    wasser_mean_loss = torch.mean(torch.sum(torch.sqrt(integral_diff**2), dim=-1))/C
    return wasser_mean_loss