...
in_param_list = [LOAD_PATH, DEVICE, MODEL_TYPE, DATASET_TYPE, BOTTLENACK, MODEL_DIR, MODEL_NAME]
in_param__names_list = ['LOAD_PATH', 'DEVICE', 'MODEL_TYPE', 'DATASET_TYPE', 'BOTTLENACK', 'MODEL_DIR', 'MODEL_NAME']
...
in_param_list = [BOTTLENACK, OUT_FEATURES, NONLINEARITY, IN_FEATURES,  N_BINS, DROPOUT, TEMP, SAMPLING]
in_param__names_list = ['BOTTLENACK', 'OUT_FEATURES', 'NONLINEARITY', 'IN_FEATURES',  'N_BINS', 'DROPOUT', 'TEMP', 'SAMPLING']
print_params(in_param_list, in_param__names_list)
print()
...
if MODEL_TYPE == 'LRAE':
    ...
    model = ConvLRAE(IN_FEATURES, BOTTLENACK, OUT_FEATURES, N_BINS, GRID, dropout=DROPOUT, nonlinearity=NONLINEARITY,
                sampling=SAMPLING, temperature=TEMP, in_channels=DS_IN_CHANNELS).to(device)
elif MODEL_TYPE == 'VAE':
    model = ConvVAE(IN_FEATURES, BOTTLENACK, OUT_FEATURES, nonlinearity=NONLINEARITY, in_channels=DS_IN_CHANNELS).to(device)
elif MODEL_TYPE == 'AE':
    model = ConvAE(IN_FEATURES, BOTTLENACK, OUT_FEATURES, nonlinearity=NONLINEARITY, in_channels=DS_IN_CHANNELS).to(device)
else:
